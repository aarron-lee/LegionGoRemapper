import { ToggleField, SliderField, PanelSection, ServerAPI, Dropdown, DropdownItem, TextField, ButtonItem, Button } from 'decky-frontend-lib';
...
const ControllerLightingPanel: VFC<{ serverAPI: ServerAPI }> = ({ serverAPI }) => {

  const [ledRightBrightness, setRightLedBrightness] = useState(DEFAULT_STATE.ledR_b);
  const [ledLeftBrightness, setLeftLedBrightness] = useState(DEFAULT_STATE.ledL_b);
  ...
  const handleSliderChange = (controller: string, newValue: number) => {
    let r = 100; let g = 100; let b = 100;
    if (controller === 'LEFT') {
      setLeftLedBrightness(newValue);
      r = redL; g = greenL; b = blueL;
    } else if(controller === 'RIGHT') {
      setRightLedBrightness(newValue);
      r = redR; g = greenR; b = blueR;
    }
    console.log(`New brightness: ${newValue}`);
    serverAPI!.callPluginMethod('rgb_brightness', {"controller": controller, "value_str": newValue, "red": r, "blue": b, "green": g},);
  ...
  const TPadToggleChange = (value:boolean) => {
    setIsTouchpad(value);
    console.log(`Toggle value: ${value}`);
    serverAPI!.callPluginMethod("touchpad_toggle", {"enable": value})
  }
  ...
  const handleColorChange = (controller: string) => {
    ...
    serverAPI!.callPluginMethod("rgb_color", {"controller": controller, "red": r, "blue": b, "green": g, "brightness": ledB})
  ...
  return (
    <PanelSection title="Controller Lighting">
      <div>
        ...
        {isRightRgbOn && (
        <><Button style={{ width: '100%' }} onClick={() => setShowRightOptions(!showRightOptions)}>
        ...
        />
        {isLeftRgbOn && (
        <><Button style={{ width: '100%' }} onClick={() => setShowLeftOptions(!showLeftOptions)}>
